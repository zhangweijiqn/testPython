...
x[x > 0.5]
# 下标还可以用来修改元素的值
# 对于一维数组而言：[start:end]是取从start到end(不包括end)索引的元素，[start:end:step]是每隔step步长取start到end索引的元素
# 对于多维数组而言，上面的操作是对第一维(行)的操作，相当于省略了其他维度的操作，[start:end,start:end,...]
a[2:4] = 100, 101
a[1:-1:2]  # 范围中的第三个参数表示步长，2表示隔一个元素取一个元素
a[5:1:-2]  # 步长为负数时，开始下标必须大于结束下标
a[::-1]  # 反转a,省略范围的开始下标和结束下标，步长为-1，整个数组头尾颠倒
a[:]  # 省略开始结束下标，取所有元素的值
